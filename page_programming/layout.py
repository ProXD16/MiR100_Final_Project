...
class LayoutManager:
    ...
    DEFAULT_PROGRAMS_FILE_PATH = "database_json/programming_json.json"
    DEFAULT_POSITIONS_FILE_PATH = "database_json/position_marker.json"
    DEFAULT_QUEUE_FILE_PATH = "database_json/queue_programming.json"
    ...
    def create_battery_command_card_layout(self, command_info):
            ...
            dbc.Row([
                ...
                dbc.Col([
                    dbc.Button(html.I(className="fas fa-cog"), 
                               id={"type": "command-config-btn", "command_id": command_id, "command_type": "battery", "subtype": subtype}, 
                               color="light", outline=True, size="sm", 
                               style={"borderRadius": "50%", "width": "35px", "height": "35px", 
                                      'display': 'none' if subtype == 'charging' else 'inline-block'}), # Hide cog for charging subtype
        ...
        if subtype == "charging":
            target_level = command_info.get("config", {}).get("target_battery_level", 80) # Default to 80 if not set
            charging_config_input = html.Div([
                html.Hr(style={'margin': '0.5rem 0'}),
                dbc.Row([
                    dbc.Col(dbc.InputGroup([
                        dbc.InputGroupText("Target Level (%)", style={'fontSize': '0.85rem'}),
                        dbc.Input(
                            id={"type": "battery-charge-target-level", "command_id": command_id},
                            type="number",
                            value=target_level,
            ...
            card_body_children.append(charging_config_input)
    ...
    def create_math_command_card_layout(self, command_info):
        ...
        # SỬA ĐỔI Ở ĐÂY: Thêm limit_one=True
