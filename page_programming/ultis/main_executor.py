# --- START OF FILE main_executor.py ---

#!/usr/bin/env python
"""
Main executor for the MIR100 robot program, handling navigation, battery status, and subprogram execution.
Integrates with BatteryChecker for battery level management based on simulation or real mode.
"""
...
from battery.simulated_charger import SimulatedCharger
from battery.real_charger import ChargeAtStation

class RobotProgramExecutor(LogicGateExecutor):
    """
    Executes robot programs, including movement, trajectory following, and battery status checking.
    Inherits from LogicGateExecutor for logic and condition evaluation.
    """
    def __init__(self, marker_json_path, 
                 ...
                 simulation_state_path="database_json/simulation_state.json"):
        """
        Initialize the RobotProgramExecutor.

        Args:
            marker_json_path (str): Path to marker JSON file for navigation.
            sub_programs_json_path (str): Path to subprograms JSON file.
        ...
        """
        super().__init__()
        # Initialize battery checker for real/simulation mode battery management
        ...
        # Initialize navigation to markers
        ...
        # Initialize chargers
        self.simulated_charger = SimulatedCharger(
            docker_json_path=os.path.join(os.path.dirname(marker_json_path), "docker.json"), # Ensure docker.json is in the same dir
            simulation_state_path=simulation_state_path,
            battery_file_path=battery_file_path,
            move_obstacle_threshold=move_obstacle_threshold,
            move_obstacle_check_angle=move_obstacle_check_angle,
            front_lidar_topic=front_lidar_topic,
            back_lidar_topic=back_lidar_topic,
            measure_distance_angle_deg_total=measure_distance_angle_deg_total
        )
        self.real_charger = ChargeAtStation()
        ...
        # Determine simulation or real mode
    ...
    def _load_simulation_state(self):
        """
        Load simulation state from JSON file to determine if running in simulation or real mode.
        Returns True for simulation, False for real mode.
        """
        try:
            with open(self.simulation_state_path, 'r') as f:
                state = json.load(f)
            rospy.loginfo(f"Simulation state file contents: {state}")
            # Check for 'mode' key used by BatteryChecker, or fall back to 'simulation_running'
            if "mode" in state:
                simulation_running = state["mode"].lower() != "real"
            else:
            ...
            rospy.logwarn(f"Failed to load simulation state from {self.simulation_state_path}: {e}. Defaulting to real mode.")
            return False  # Default to real mode

    def _pose_callback(self, msg):
        """
        Callback for robot pose updates.
        Updates self.current_pose with [x, y, yaw] in simulation or real mode.
        """
        if self.simulation_mode != 'real':
    ...
    def _load_sub_programs(self):
        """
        Load subprograms from JSON file into a dictionary.
        Returns a dictionary mapping program names to command lists.
        """
    ...
    def _get_value_from_command_source(self, value_command):
        """
        Retrieve a value from a command source (e.g., battery level, distance).
        ...
        if cmd_type == "battery" and cmd_subtype == "status":
            battery_level = self.battery_checker.check_battery()
            if battery_level is not None:
                rospy.loginfo(f"RobotExecutor (CmdID {cmd_id}): Battery level: {battery_level}%")
                return float(battery_level)
            else:
                rospy.logwarn(f"RobotExecutor (CmdID {cmd_id}): Failed to retrieve battery level.")
                return None
    ...
    def evaluate_conditions(self, conditions_block):
        """
        Evaluate a list of conditions (logic or math-based).
        ...
        for item in conditions_block:
            ...
            type_val = item.get("type") # Renamed from 'type' to avoid conflict with built-in
            subtype = item.get("subtype")
            config = item.get("config", {})
            rospy.logdebug(f"RobotExecutor (CondID {cond_id}): Evaluating item: type='{type_val}', subtype='{subtype}'")
            ...
            if type_val == "logic":
                ...
                val_a_src_list = config.get("value_a_commands") # Changed from value_a_source
                val_a = None
                if val_a_src_list and isinstance(val_a_src_list, list) and len(val_a_src_list) > 0:
                    val_a = self._get_value_from_command_source(val_a_src_list[0])
                else: # Fallback if value_a_commands is missing or empty, check for direct value_a
                    val_a = config.get("value_a")

                if val_a is not None and not isinstance(val_a, (int, float)):
                    try:
                        val_a = float(val_a)
                    except (ValueError, TypeError):
                        rospy.logerr(f"RobotExecutor (CondID {cond_id}): Direct value_a '{val_a}' not numeric.")
                        return False
                if val_a is None:
                ...
                try:
                    ...
                    rospy.logerr(f"RobotExecutor (CondID {cond_id}): value_b '{val_b}' not numeric.")
                    return False
                ...
                rospy.logwarn(f"RobotExecutor (CondID {cond_id}): Unknown condition type '{type_val}'. FALSE.")
                return False
    ...
    def execute_command(self, command):
        """
        Execute a single command (e.g., move, trajectory, battery, log, subprogram).
        Returns None for success, or a signal (e.g., SIGNAL_FATAL_ERROR) for failure.
        """
        ...
        # Check base class for logic-related commands
        ...
        if base_result is False: 
            ...
            if command_type == "move": 
                ...
                if marker_id is not None:
                    ...
                    if self.move_to_marker.send_goal(marker_id):
                        ...
                        if move_result is GOAL_COMPLETION_OBSTACLE:
                            ...
                            # Depending on strictness, this could be a FATAL_ERROR too
                            return None # For now, allow program to continue
                    else:
                        rospy.logerr(f"RobotExecutor (CmdID {command_id}): Failed to send goal for marker_id: {marker_id}.")
                        return SIGNAL_FATAL_ERROR # Cannot send goal is more critical
                else:
                ...
                if marker_id is not None:
                    ...
                    start_time_wait_pose = rospy.Time.now().to_sec() # Renamed variable
                    timeout_wait_pose = 5.0 # Renamed variable
                    while self.current_pose is None and (rospy.Time.now().to_sec() - start_time_wait_pose) < timeout_wait_pose:
                        rospy.sleep(0.1)
                ...
                if subtype == "charging":
                    target_battery_level = config.get("target_battery_level", 100.0) # Default to 100%
                    rospy.loginfo(f"Target battery level for charging: {target_battery_level}%")
                    if self.simulation_mode:
                        rospy.loginfo(f"RobotExecutor (CmdID {command_id}): Executing simulated charging to {target_battery_level}%")
                        success = self.simulated_charger.charge_battery(target_battery_level=target_battery_level)
                        if not success:
                            rospy.logerr(f"RobotExecutor (CmdID {command_id}): Simulated charging failed.")
                            return SIGNAL_FATAL_ERROR
                        rospy.loginfo(f"RobotExecutor (CmdID {command_id}): Simulated charging completed.")
                    else:
                        rospy.loginfo(f"RobotExecutor (CmdID {command_id}): Executing real charging (to target {target_battery_level}% - NOTE: Real charger might not support specific target level via this script, uses MiR mission).")
                        self.real_charger.set_robot_ready()
                        result = self.real_charger.queue_charge_mission() # Real charger likely charges to full via MiR mission
                        if "✅" in result: # Simple check for success message
                            rospy.loginfo(f"RobotExecutor (CmdID {command_id}): {result}")
                            # In real mode, we might not wait here for the full charge.
                            # The MiR handles the charging. For the script, queuing the mission is success.
                            # If we need to wait until target_battery_level, more complex API interaction with MiR status is needed.
                        else:
                            rospy.logerr(f"RobotExecutor (CmdID {command_id}): Real charging failed: {result}")
                            return SIGNAL_FATAL_ERROR
                    return None
                elif subtype == "status":
                    battery_level = self.battery_checker.check_battery()
                    rospy.loginfo(f"RobotExecutor (CmdID {command_id}): Battery status: {battery_level}%")
                    return None
                else:
        ...
        return None # Should not be reached if base_result is True or None

    def execute_program(self, json_data_list):
        """
        Execute a list of programs from JSON data.
        Each program contains a list of commands to execute.
        """
        ...
        for program_idx, program_data in enumerate(json_data_list):
            ...
            for cmd_idx, command_item in enumerate(commands):
                if rospy.is_shutdown():
                    ...
                    # Break from inner loop, outer loop will check rospy.is_shutdown() again
                    break 
...
def main():
    """
    Main function to initialize the ROS node and execute programs from the queue JSON file.
    """
    ...
    try:
        ...
        battery_file_path = os.path.join(base_path, "battery_percent.json")
        ...
        move_obstacle_thresh_param = rospy.get_param("~move_obstacle_threshold", 0.1)
        ...
        rospy.loginfo(f"Battery file: {battery_file_path}")
        ...
        # Create dummy marker file if not found
        if not os.path.exists(marker_json_path):
            ...
            dummy_markers = [
                ...
                {"id": 2, "name":"Marker2", "x": 1.5, "y": 0.5, "w": 0.707, "z": 0.707}, # Added Marker2
                {"id": 3, "name":"Marker3", "x": -1.0, "y": 0.5, "w": 0.707, "z": 0.707},
                {"id": 4, "name":"Marker4", "x": 0.0, "y": 1.0, "w": 0.0, "z": 1.0},     # Added Marker4
            ...
            try:
                ...
                if not os.path.exists(marker_dir) and marker_dir: # Ensure directory exists
        ...
        executor = RobotProgramExecutor(
            ...
            battery_file_path=battery_file_path,
            front_lidar_topic=front_lidar_param if front_lidar_param else None, # Pass None if empty
            back_lidar_topic=back_lidar_param if back_lidar_param else None,   # Pass None if empty
        ...
        # Load and execute main program queue
        ...
        rospy.logfatal(f"An unexpected error in main of '{node_name}': {e}", exc_info=True) # exc_info=True for traceback
    finally:
