...
# Global variables for simulated battery
_sim_battery_level = 100.0
...
def update_battery_status(n):
    global _sim_battery_level, _last_battery_update
    ...
    if mode == "real":
        ...
        if _last_battery_update is None:
            _sim_battery_level = _load_simulated_battery(battery_file_path)
            if _sim_battery_level <= 0.0:
                _sim_battery_level = 100
            _last_battery_update = time.time()
        current_time = time.time()
        if (current_time - _last_battery_update) >= 30.0:
            _sim_battery_level = max(0.0, _sim_battery_level - 1.0)
            _last_battery_update = current_time
            _save_simulated_battery(_sim_battery_level, battery_file_path)
            print(f"Simulation mode: Battery level decreased to {_sim_battery_level}%")
        return f"{round(_sim_battery_level)}%"
...
def update_battery_icon(percentage):
    try:
