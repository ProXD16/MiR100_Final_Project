...
class MPCController:
    ...
    def get_derivative_on_path(self, global_s):
        ...
        if ds_dt_local < 1e-6:
            # Try nearby points to estimate derivative
            if t_local < 0.5:
...
def main():
    ...
    try:
        ...
        splines = [item for item in json_data if item.get("type") == "spline5"]
        if not splines:
            rospy.logerror("No 'spline5' objects found in JSON file.")
            return
