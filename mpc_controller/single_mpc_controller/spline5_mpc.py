...
def main():
    ...
    try:
        ...
        for item in json_data:
            if item.get("type") == "spline5":
                controller.waypoints = [np.array([point["x"], point["y"]]) for point in item["points"]]
                initial_goal = np.array([item["startPoint"]["x"], item["startPoint"]["y"]])
                break
        else:
            rospy.logerror("No 'spline5' object found in JSON file.")
            return
        rospy.loginfo(f"Loaded {len(controller.waypoints)} waypoints from {json_path}")
    except Exception as e:
        rospy.logerror(f"Failed to load JSON file {json_path}: {e}")
        return

    if len(controller.waypoints) < 2:
        rospy.logerror("Need at least two waypoints to create a path.")
        return
    ...
    rospy.loginfo(f"Sent initial goal to move_base: ({initial_goal[0]:.2f}, {initial_goal[1]:.2f})")
    ...
    if not controller.calculate_path():
        rospy.logerror("Failed to create path from waypoints.")
        return
